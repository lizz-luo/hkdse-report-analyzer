...
st.set_page_config(page_title="自定義MCQ分析", page_icon="📌", layout="wide", initial_sidebar_state="expanded")
st.title("📌 自定義MCQ分析 app")
...
if not df_item_c.empty:
    ...
    for i, col in enumerate(st.session_state.custom_cols):
        with f_cols[i]:
            ...
            active_filters[col] = st.multiselect(f"篩選 {col}", u_vals, key=f"filter_mcq_{col}")

    c4, c5 = st.columns([2, 1])
    with c4:
        sort_by = st.selectbox("排序依據", ["預設（按題號）", "Your school Mean %", "Day schools Mean %"], key="sort_mcq")
    with c5:
        sort_order = st.radio("排序方式", ["由高至低", "由低至高"], horizontal=True, key="order_mcq")
